...
##@@self.target_parameters = ["time"]
    self.target_parameters = ["Event count"]
...
#@@          print("##########@@@@@@@@@@@ data[target_param]=", data[target_param])
  ...
  def parse_data_from_dir(self, target_log_dir):
    ...
    for _dir in os.listdir(target_log_dir):
      for path, directories, files in os.walk(os.path.join(target_log_dir,_dir)):
        print("@@@@@@@@@@@@@@ _dir = ", _dir, "path = ", path, "directories = ", directories, "files = ", files)
        #get setting parameter first,
        parameter = []
        data = {}

##&&&&&&&&&&&&&&&&&&&&&&        if not(os.path.isfile(os.path.join(path,"perf_output"))):
#@@print("#!#!#!#!#!#!#!#!#!#! Can't find perf_output")
##&&&&&&&&&&&&&&&&&&&&&&          continue
##&&&&&&&&&&&&&&&&&&&&&&        else :
##&&&&&&&&&&&&&&&&&&&&&&          print("#!#!#!#!#!#!#!#!#!#! find perf_output")
  ###########@@@@@@@@@@@@@@@      perf_output = open(os.path.join(path, "perf_output"))
        if os.path.isfile(os.path.join(path,"perf_output")):
          perf_output = open(os.path.join(path, "perf_output")).read()

          i = 0
          for para in list(map(lambda x: x.strip().split(": "),perf_output[0:-1].splitlines())):
            print("$$$$$$$$$$$$$$$$$$$$$ para =", para)
            data[para[0]] = para[-1]
            print("$$$$$$$$$$$$$$$$$$$$$ data =", data)
            i = i + 1
...
#@@print("@@@@@@@@@@@@@@@@@@@@ perf_line = ", perf_line)
 ###########@@@@@@@@@@@@@@@         if perf_line.find("count") != -1:
 ###########@@@@@@@@@@@@@@@           perf_count = perf_line[13:30]
#@@            print("#!#!#!#!#!#!#!#!#!#! perf_count = ", perf_count)
 ###########@@@@@@@@@@@@@@@           break
#@@          else :
#@@            print("#!#!#!#!#!#!#!#!#!#! can't find count")

#!#!        print("@@@@@@@@@@@@@@@@@@@ perf_cout = ", perf_count)

###########@@@@@@@@@@@@@@@
###########@@@@@@@@@@@@@@@
###########@@@@@@@@@@@@@@@
 ###########@@@@@@@@@@@@@@@       if not(os.path.isfile(os.path.join(path,"setting"))):
 ###########@@@@@@@@@@@@@@@         continue
...
'''
for target_dir in ["logs_9","logs_99101FFBA006XF", "logs_9A221FFBA0002G", "logs_9B061FFBA000ST"]:
...
#@@train_target_dirs =["train_dataset"]
#@@test_target_dirs =["test_dataset"]
...
#@@train_target_dirs =["train_dataset/iu", "train_dataset/kookyohwan", "train_dataset/parkjunghun"]
#@@test_target_dirs =["test_dataset"]
#@@test_target_dirs =["test_dataset/onebeen"]
#@@train_target_dirs =["train_dataset/yunakim", "train_dataset/yunakim"]
#@@test_target_dirs =["test_dataset/yunakim"]

#@@train_target_dirs =["train_dataset/iu", "train_dataset/kookyohwan"]
#@@test_target_dirs =["test_dataset/parkjunghun"]

train_target_dirs =["perf_dataset"]
test_target_dirs =["perf_dataset"]
...
print("#!#!#!#!#![main] len(train_data)=", len(train_data), "len(test_data)=", len(test_data))
print("#!#!#!#!#![main] train_data=", train_data)
#@@print("#!#!#!#!#![main] test_data=", test_data)
