...
for target_dir in target_dirs:
  total_data.extend(model.parse_data_from_dir(os.path.join("logs/logs_0B131JEC200794",target_dir)))
...
for idx in range(len(total_data)):
  ...
  print("time=", time)
#!#!  target_start_time = 3 #in min [0, 10, 20, 30]	#@@

#!#!  if not ((time > (target_start_time * 60)) and (time <= ((target_start_time + 10) * 60))):	#@@
#!#! #@@    print("#@@time=", time)
#!#!    continue
#!#!  else :
#!#!    print("#!#!#!#!#! time=", time)
