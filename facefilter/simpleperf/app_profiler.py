...
class ProfilerBase(object):
    ...
    def __init__(self, args):
        ...
        if(hasattr(args, 'd')):
          self.adb = AdbHelper(enable_switch_to_root=not args.disable_adb_root,device_id=args.d)
        else:
    ...
    def start_profiling(self, target_args):
        ...
        args += target_args
        adb_args = [self.adb.adb_path, '-s', self.device_id, 'shell'] + args
    ...
    def collect_profiling_data(self):
        ...
        if not self.args.skip_collect_binaries:
            ...
            binary_cache_args += ['-d', self.device_id]
            if self.args.native_lib_dir:
...
def main():
    ...
    other_group.add_argument('-d', type=str
        )
